...
TABLE_SUMMARY_PROMPT_KO = f"""
이 이미지는 표, 행렬, 지표 목록, 계획 과제 목록 또는 수치 요약일 수 있습니다.
표 제목, 열/행 헤더, 단위, 기간, 수치 범위, 비교 항목, 강조 표시된 셀과 주석을 우선 추출하세요.
OCR로 읽기 어려운 셀도 눈에 보이는 구조와 핵심 라벨을 검색 가능한 근거로 정리하세요.

{STRUCTURED_VISUAL_JSON_CONTRACT_KO}
""".strip()


CHART_SUMMARY_PROMPT_KO = f"""
이 이미지는 차트, 그래프, 시계열, 막대/선/원형 그래프 또는 지표 시각화일 수 있습니다.
차트 제목, 축 이름, 단위, 범례, 색상 의미, 주요 추세, 최고/최저 값, 비교 관계를 우선 추출하세요.
정확한 숫자가 불확실하면 보이는 범위와 상대적 증감 관계를 명확히 구분하세요.

{STRUCTURED_VISUAL_JSON_CONTRACT_KO}
""".strip()


FIGURE_SUMMARY_PROMPT_KO = f"""
이 이미지는 다이어그램, 개념도, 프로세스 그림, 사진, 삽화 또는 복합 시각 자료일 수 있습니다.
구성 요소, 흐름 방향, 단계, 라벨, 연결선, 공간 배치, 사진 속 주요 객체와 설명 문구를 우선 추출하세요.
객체나 영역이 검색 단서가 될 수 있으면 objects에 라벨, 위치, 속성, bbox를 함께 넣으세요.
