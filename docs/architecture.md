...
- `eval-retrieval`: focused top-k retrieval benchmark cases with optional repeated latency and result-stability sampling, target-specific page/chunk/asset/triple metrics, hard-negative excluded-target hit metrics, visual asset provenance matching for triple targets, source-family contribution metrics, chunking-strategy or retrieval-role contribution metrics, and case metadata group metrics such as `case_source` or `query_mode`.
