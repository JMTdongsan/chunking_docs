...
- `gate-qdrant-vector-ablation`: pass/fail checks for a selected Qdrant vector mode using recall, target coverage, target nDCG, precision, failed-query count, latency, source-family target coverage, and optional best-mode requirements.
...
- `gate-retrieval`: pass/fail checks for absolute metric floors, source-family target coverage, and baseline regression limits such as recall drop, target coverage drop, target nDCG drop, precision drop, and latency ratio.
