...
- `gate-retrieval`: pass/fail checks for absolute metric floors, target-type coverage, source-family and exact source target coverage, chunking-strategy coverage, retrieval-role coverage, case metadata group coverage, and baseline regression limits such as recall drop, target coverage drop, target nDCG drop, precision drop, and latency ratio.
