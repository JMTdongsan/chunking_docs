...
- `gate-qdrant-vector-ablation`: pass/fail checks for a selected Qdrant vector mode using recall, target coverage, target nDCG, target rank limits, precision, failed-query count, latency, hard-negative excluded-target hit rates, source-family, exact-source, chunking-strategy, and retrieval-role excluded-target hit rates, target-type coverage, source-family and exact source target coverage, case metadata group coverage, case-group exact-source and source-family target coverage, optional best-mode requirements, pairwise rank-delta ceilings, and query-paired baseline thresholds when a baseline mode is supplied.
- `gate-qdrant-reranker-ablation`: pass/fail checks for a selected Qdrant reranker mode using the same retrieval, target, source, case-group, case-group source contribution, pairwise, rank-delta, latency, and best-mode checks while preserving reranker source and rerank depth in the report.
