...
2. **Page Profiling**
   ...
   - Convert those recommendations into an ordered ingestion workflow plan that records runtime checks, profile-specific VLM runtime outputs, VLM experiment output gates, visual processing outputs, annotation application, embedding rebuilds, retrieval cases, chunking comparison, and final readiness commands.
...
7. **OCR/VLM Job Planning**
   ...
   - Build VLM experiment recipes that only require OCR dependencies when selected jobs contain OCR work, and require Paddle CUDA only for GPU OCR, keeping CPU OCR plus GPU VLM profile comparisons runnable on standard Paddle installs.
   - Write VLM experiment plans so several profiles can be run against the same visual job set and compared afterward, including selected job counts, operation mix, per-profile runtime checks with reusable `runtime_doctor.<profile>.json` outputs, batch offsets and limits, batch merge commands, batch comparison commands, and generation-token upper bounds for local GPU sizing.
...
9. **Strategy Variants**
   ...
   - `apply-chunking-sweep` promotes a reviewed sweep recommendation into the package chunk file, remaps triples, refreshes BM25, clears stale vector artifacts, and records the selected chunking candidate before dense embeddings are rebuilt.
...
- `object_dense`: one detected visual object, region, or visual element per record, including label, attributes, description, location, bbox region, confidence, source field, feature type, and parent asset provenance.
...
12. **Package Reproducibility**
    ...
    - Existing package metadata can be refreshed without rebuilding chunks or vectors when old manifests predate the reproducibility contract.
    - `embedding_manifest.json` records vector files, dimensions, counts, checksums, backend names, model IDs, devices, and batch sizes.
...
14. **Storage**
    ...
    - PostgreSQL stores normalized document, page, chunk, asset, visual object, triple, embedding artifact, and embedding record metadata.
...
The package writes payload index definitions with field schemas. Qdrant ingestion and package query commands apply those definitions so metadata filters such as document ID, asset ID, object ID, object label, bbox region, visual feature type, page, section, chunking strategy, hierarchy role, text quality, OCR/VLM work flags, visual asset scope, tile parent/grid fields, and standalone visual assets remain efficient on server-backed collections.
...
The writer upserts in dependency order: documents, pages, chunks, BM25 token rows, assets, visual objects, chunk-to-asset links, triples, embedding artifacts, embedding records, and embedding vector summaries. `chunk_lexical_tokens` mirrors `bm25_tokens.json` with tokenizer configuration, token counts, and token arrays, which keeps lexical retrieval experiments reproducible and leaves room for a PostgreSQL-backed lexical service later. Chunk metadata preserves visual asset IDs derived from both direct asset links and `asset:` source refs, and `chunk_asset_links` stores the same relationships in normalized rows for joins and audits. `visual_objects` normalizes structured VLM object, region, and visual-element metadata from assets into object-level rows with labels, bbox regions, attributes, descriptions, confidence, object text, source field, feature type, and asset/page provenance so object-detection probes can be joined and audited without parsing nested asset JSON. Asset-backed graph triples are remapped to an available chunk before PostgreSQL rows are written, preserving the original chunk ID in qualifiers so VLM-derived triples satisfy relational foreign keys without losing provenance. The `embedding_artifacts` table stores vector file names, dimensions, counts, checksums, Qdrant collection names, backend/model metadata, payload index fields, and payload index schemas from `embedding_manifest.json`; `embedding_records` mirrors Qdrant JSONL records into pgvector-compatible rows with payload, target kind, target ID, vector family, and dimension metadata. `embedding_vector_summaries` groups those records by document, vector family, and target kind so Postgres can report text, caption, image, object, and triple coverage without scanning every vector row. Ingestion readiness compares summaries with artifact dimensions, artifact record counts, and known vector-family target kinds before database upsert. The schema does not create a default ANN index because vector families can have different dimensions; deployment databases can add per-family indexes after choosing a serving strategy.

`docker-compose.postgres.yml` provides a local PostgreSQL 16 runtime with pgvector enabled for development and ingestion rehearsals. Its data directory defaults to `.local/postgres`, which keeps database state outside the public package and source tree. `postgres-schema` exports the SQL contract for review or migration tooling without requiring a live database. `postgres-check-schema` validates the live PostgreSQL schema before upsert. It checks required tables, columns, column types, relational/search indexes, and the pgvector extension so metadata ingestion failures and slow-path schema drift are caught before batch writes.
...
- `eval-chunking`: page coverage, chunk size distribution, section coverage, visual linkage, annotation coverage, retrieval recall@k, MRR, target coverage@k, target nDCG@k, precision@k, latency, retrieval-per-embedding-cost metrics, retrieval-per-latency metrics, failed queries, and aggregate quality score.
- `audit-retrieval-cases`: benchmark case validation for empty or TODO queries, weak short queries, unknown expected or excluded page/chunk/asset/triple targets, duplicate queries, graph-expansion hints, target-family coverage, distinct target coverage, case-group distinct target coverage, per-target concentration limits, per-case expected target ceilings, target-query overlap ratio and term-count leakage limits, required case metadata group counts such as visual object probes, and optional enforcement that object probes use visual-only VLM/object terms.
- `eval-retrieval`: focused top-k retrieval benchmark cases with optional repeated latency and result-stability sampling, target-specific page/chunk/asset/triple metrics, hard-negative excluded-target hit metrics, exact-source and source-family contamination metrics, visual asset provenance matching for triple targets, source-family contribution metrics, chunking-strategy or retrieval-role contribution metrics, and case metadata group metrics such as `case_source` or `query_mode`.
- `generate-retrieval-cases`: benchmark draft generation from package pages, candidate chunk files, visual assets, graph triples, optional visual lexical probes, rendered-image probes for `image_dense`, and VLM object-detection or visual-element probes, with snippet, document-frequency-weighted salient-term, or natural-question query modes, optional target-query leakage and per-target concentration filters, generated same-kind hard-negative excluded targets, visual-only object probe terms by default, and visual asset targets for asset-provenance triples.
- `diagnose-retrieval`: failure, partial-coverage, low-ranking, low-precision, and hard-negative source analysis for retrieval evaluation JSON outputs, including case metadata, exact-source and source-family counts and match rates for top hits, matched expected targets, and excluded-target hits, plus case-group source and failure breakdowns for benchmark subsets such as visual object probes.
- `eval-qdrant-retrieval`: the same benchmark cases against Qdrant named vectors plus BM25 and optional graph expansion. Query encoder `auto` mode reads `embedding_manifest.json` so model-backed package vectors are queried with the matching text or CLIP text-side encoder when available.
- `eval-qdrant-retrieval-config`: reloads an exported Qdrant retrieval config and evaluates benchmark cases with its vector names, optional adaptive routes, fusion weights, graph expansion, hierarchy collapse, tokenizer settings, reranker settings, selection metadata, and manifest-derived query encoders. Routed configs prepare the union of default and route-specific vectors, then choose object, triple/graph, or default settings per query. Each routed evaluation records per-query route decisions and adds `retrieval_route`, `retrieval_route_reason`, and route-by-source contribution metrics so route-specific target coverage can be gated against the intended vector source, such as `qdrant:object_dense` for visual-object routes or `qdrant:triple_dense` for graph-triple routes.
- `qdrant-rag-context-config`: reloads an exported Qdrant retrieval config and builds citation-ready context with the same vector names, fusion weights, graph expansion, hierarchy collapse, tokenizer settings, reranker settings, selection metadata, and manifest-derived query encoders.
...
- `plan-ingestion-workflow`: ordered package-specific command plan from characterization through OCR/VLM, profile-specific runtime doctor outputs, VLM experiment plan gates, profile-specific visual result files, visual run comparison gates, annotation application, retrieval cases, retrieval-quality/cost/latency-gated chunking comparison, index refresh, embedding rebuilds, Qdrant retrieval config export, final RAG context gates, metadata refresh, and ingestion readiness with the runtime report attached. When `--retrieval-cases` is omitted, generated benchmark cases stay under the package directory by default. When object and triple vector families are both recommended, the exported Qdrant config uses the adaptive route preset and the same routed profile is carried through config retrieval evaluation, RAG context evaluation, and readiness, including route-level exact-source, source-family contribution, and hit-precision gates.
...
- `gate-retrieval-ablation`: pass/fail checks for a selected retrieval ablation mode using absolute thresholds, baseline lift, target rank limits, target-type coverage, source-family and exact source coverage and hit precision, case metadata group coverage, case-group exact-source and source-family target coverage and hit precision, hard-negative excluded-target hit rates, source-family, exact-source, chunking-strategy, and retrieval-role excluded-target hit rates, strategy/role contribution metrics, best-mode requirements, latency limits, pairwise rank-delta ceilings, and query-paired baseline thresholds.
- `gate-retrieval`: pass/fail checks for benchmark size, expected target count, passed and failed query counts, absolute metric floors, repeated result stability, target-type coverage, source-family and exact source target coverage, source-family and exact-source hit precision, source-family, exact-source, chunking-strategy, and retrieval-role excluded-target hit rates, chunking-strategy coverage, retrieval-role coverage, case metadata group coverage, and baseline regression limits such as recall drop, target coverage drop, target nDCG drop, precision drop, and latency ratio.
...
- `compare-chunking`: side-by-side strategy comparison by quality score, recall@k, MRR, target coverage@k, target nDCG@k, target rank, precision@k, target-type coverage, exact-source and source-family target coverage, case-group exact-source and source-family target coverage, chunking-strategy coverage, retrieval-role coverage, case group coverage, linked visual text asset and part coverage, chunk text volume, retrieval-per-embedding-cost metrics, retrieval-per-latency metrics, latency, failed queries, query-paired baseline deltas, and paired bootstrap confidence intervals.
- `gate-chunking-comparison`: pass/fail checks for selected chunking candidates using quality, page coverage, visual text asset/part coverage, retrieval floors, target rank limits, target-type coverage, exact-source and source-family target coverage, exact-source and source-family hit precision, case-group exact-source and source-family target coverage and hit precision, chunking-strategy coverage, retrieval-role coverage, case group coverage, failed-query limits, chunk text volume limits, retrieval-per-embedding-cost floors, retrieval-per-latency floors, baseline regression limits, pairwise lift requirements, pairwise rank-delta ceilings, and paired confidence bounds.
- `sweep-chunking`: parameter grid generation for max size, overlap, parent size, and multimodal, object-aware, or hierarchical visual context size, with weighted selection scores, optional hard selection constraints for aggregate metrics, visual text asset and part coverage, result stability, target types, source families, case groups, chunk text volume, chunk length, standalone visual chunk count, visual object chunk count, retrieval-per-embedding-cost metrics, and retrieval-per-latency metrics, eligibility failures, and a Pareto front that treats mean/p95 latency, rank, result instability, chunk count, chunk text volume, embedding text volume, standalone visual chunk count, and visual object chunk count as cost axes for retrieval quality-versus-cost review.
- `apply-chunking-sweep`: applies a selected sweep candidate to `chunks.jsonl`, writes backups, remaps triples to available chunk IDs, rebuilds BM25 tokens, clears stale Qdrant record, collection, and embedding manifest files unless deterministic dry-run embeddings are rebuilt, and updates package metadata with the selected strategy and candidate config.
- `write-experiment-report`: reproducible package report with source-file/package config metadata, artifact checksums, record counts, tokenizer settings, Qdrant collection and exported retrieval configuration, Qdrant fusion case-group recommendation metrics, readiness, evaluation, audit, gate artifact variants, visual run comparison summaries, chunking sweep recommendation summaries, top-level and component-level validation pass/fail summaries, linked visual text asset/part coverage, target rank metrics, case metadata group metrics, retrieval diagnostics reason/source/source-family counts and match rates for all hits, matched expected targets, and excluded targets, paired confidence metrics, retrieval-per-embedding-cost metrics, retrieval-per-latency metrics, and candidate comparison metrics.
