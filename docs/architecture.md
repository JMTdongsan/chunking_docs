...
- `sweep-qdrant-fusion`: grid search over Qdrant, BM25, exact vector-source, and graph fusion weights with eligibility gates, latency-aware selection scores, case-group recommendations for benchmark subsets such as visual object probes, and a recommended production retrieval configuration.
