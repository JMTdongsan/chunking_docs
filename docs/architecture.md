...
- `audit-retrieval-cases`: benchmark case validation for empty or TODO queries, weak short queries, unknown page/chunk/asset/triple targets, duplicate queries, graph-expansion hints, target-family coverage, distinct target coverage, case-group distinct target coverage, per-target concentration limits, target-query overlap leakage limits, required case metadata group counts such as visual object probes, and optional enforcement that object probes use visual-only VLM/object terms.
