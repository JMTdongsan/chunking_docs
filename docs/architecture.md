...
9. **Strategy Variants**
   ...
   - `apply-chunking-sweep` promotes a reviewed sweep recommendation into the package chunk file, remaps triples, refreshes BM25, and records the selected chunking candidate before dense embeddings are rebuilt.
...
- `apply-chunking-sweep`: applies a selected sweep candidate to `chunks.jsonl`, writes backups, remaps triples to available chunk IDs, rebuilds BM25 tokens, and updates package metadata with the selected strategy and candidate config.
