...
- `diagnose-retrieval`: failure, partial-coverage, low-ranking, low-precision, and hard-negative source analysis for retrieval evaluation JSON outputs, including case metadata, exact-source and source-family counts for top hits, matched expected targets, and excluded-target hits, plus case-group source and failure breakdowns for benchmark subsets such as visual object probes.
