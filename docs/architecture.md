...
- `compare-chunking`: side-by-side strategy comparison by quality score, recall@k, MRR, target coverage@k, target nDCG@k, target rank, precision@k, target-type coverage, exact-source and source-family target coverage, chunking-strategy coverage, retrieval-role coverage, case group coverage, linked visual text asset and part coverage, chunk text volume, retrieval-per-embedding-cost metrics, retrieval-per-latency metrics, latency, failed queries, query-paired baseline deltas, and paired bootstrap confidence intervals.
- `gate-chunking-comparison`: pass/fail checks for selected chunking candidates using quality, page coverage, visual text asset/part coverage, retrieval floors, target rank limits, target-type coverage, exact-source and source-family target coverage, chunking-strategy coverage, retrieval-role coverage, case group coverage, failed-query limits, chunk text volume limits, retrieval-per-embedding-cost floors, retrieval-per-latency floors, baseline regression limits, pairwise lift requirements, pairwise rank-delta ceilings, and paired confidence bounds.
