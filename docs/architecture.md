...
- `write-experiment-report`: reproducible package report with source-file/package config metadata, artifact checksums, record counts, tokenizer settings, Qdrant collection and exported retrieval configuration, Qdrant fusion case-group recommendation metrics, readiness, evaluation, audit, gate artifact variants, visual run comparison summaries, chunking sweep recommendation summaries, top-level and component-level validation pass/fail summaries, linked visual text asset/part coverage, target rank metrics, case metadata group metrics, retrieval diagnostics reason counts, paired confidence metrics, retrieval-per-embedding-cost metrics, retrieval-per-latency metrics, and candidate comparison metrics.
