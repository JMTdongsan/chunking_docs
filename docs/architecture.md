...
- `diagnose-retrieval`: failure, partial-coverage, low-ranking, low-precision, and hard-negative source analysis for retrieval evaluation JSON outputs, including case metadata, exact-source and source-family counts for top hits and excluded-target hits, plus case-group source and failure breakdowns for benchmark subsets such as visual object probes.
...
- `write-experiment-report`: reproducible package report with source-file/package config metadata, artifact checksums, record counts, tokenizer settings, Qdrant collection and exported retrieval configuration, Qdrant fusion case-group recommendation metrics, readiness, evaluation, audit, gate artifact variants, visual run comparison summaries, chunking sweep recommendation summaries, top-level and component-level validation pass/fail summaries, linked visual text asset/part coverage, target rank metrics, case metadata group metrics, retrieval diagnostics reason/source/source-family counts, paired confidence metrics, retrieval-per-embedding-cost metrics, retrieval-per-latency metrics, and candidate comparison metrics.
