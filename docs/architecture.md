...
- `eval-qdrant-retrieval-config`: reloads an exported Qdrant retrieval config and evaluates benchmark cases with its vector names, optional adaptive routes, fusion weights, graph expansion, hierarchy collapse, tokenizer settings, reranker settings, selection metadata, and manifest-derived query encoders. Routed configs prepare the union of default and route-specific vectors, then choose object, triple/graph, or default settings per query. Each routed evaluation records per-query route decisions and adds `retrieval_route` and `retrieval_route_reason` case-group metrics so route-specific target coverage can be gated.
