...
- `eval-qdrant-retrieval-config`: reloads an exported Qdrant retrieval config and evaluates benchmark cases with its vector names, fusion weights, graph expansion, hierarchy collapse, tokenizer settings, reranker settings, selection metadata, and manifest-derived query encoders.
