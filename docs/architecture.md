...
- `write-experiment-report`: reproducible package report with source-file/package config metadata, artifact checksums, record counts, tokenizer settings, Qdrant configuration, readiness, evaluation, audit, gate artifact variants, visual run comparison summaries, top-level and component-level validation pass/fail summaries, linked visual text asset/part coverage, target rank metrics, case metadata group metrics, retrieval diagnostics reason counts, paired confidence metrics, and candidate comparison metrics.
