...
- `gate-qdrant-reranker-ablation`: pass/fail checks for a selected Qdrant reranker mode using the same retrieval, target, source, case-group, pairwise, rank-delta, latency, and best-mode checks while preserving reranker source and rerank depth in the report.
