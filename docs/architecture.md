...
- `compare-chunking`: side-by-side strategy comparison by quality score, recall@k, MRR, target coverage@k, target nDCG@k, target rank, precision@k, target-type coverage, source-family target coverage, chunking-strategy coverage, retrieval-role coverage, case group coverage, linked visual text asset and part coverage, chunk text volume, retrieval-per-embedding-cost metrics, retrieval-per-latency metrics, latency, failed queries, query-paired baseline deltas, and paired bootstrap confidence intervals.
