...
The report records source-file checksum, package settings, package artifact checksums, JSONL record counts, BM25 tokenizer settings, Qdrant named-vector configuration, readiness, evaluation, audit, gate artifact variants, visual run comparison summaries, top-level and component-level validation pass/fail summaries, chunking quality metrics, linked visual text coverage, retrieval recall@k, MRR, target coverage@k, target nDCG@k, target rank, precision@k, latency, failed queries, diagnostics reason counts, case-group diagnostics such as visual object probe failures, paired confidence metrics from chunking gates, and the best candidate by retrieval behavior. This makes chunking changes reviewable and repeatable before new defaults are adopted.
