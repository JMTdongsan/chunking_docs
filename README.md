...
PostgreSQL is intended for source metadata, page profiles, chunks, BM25 token artifacts, visual asset links, assets, normalized VLM object rows, graph triples, embedding artifact provenance, pgvector-compatible embedding record rows, and vector-family summary rows. Vector search is handled by Qdrant by default, while PostgreSQL stores vector file names, dimensions, counts, checksums, backend/model metadata, collection names, payloads, target IDs, target kinds, and per-family target coverage so embedding runs remain auditable or migratable.
...
chunking-docs ingestion-readiness \
  ...
  --max-retrieval-case-group-cases-per-target case_source:visual_object_probe:asset=3 \
  --min-retrieval-excluded-target-cases asset=4 \
  --min-retrieval-distinct-excluded-targets asset=4 \
  --max-retrieval-excluded-cases-per-target asset=4 \
...
chunking-docs audit-retrieval-cases examples/retrieval_cases.jsonl \
  ...
  --min-excluded-target-cases asset=4 \
  --min-distinct-excluded-targets asset=4 \
  --max-excluded-cases-per-target asset=4 \
...
chunking-docs sweep-qdrant-fusion examples/retrieval_cases.jsonl \
  ...
  --max-source-family-excluded-target-hit-rate visual=0.0 \
  --max-source-excluded-target-hit-rate qdrant:image_dense=0.0 \
  --min-source-family-precision-at-hits visual=0.5 \
  --min-source-precision-at-hits qdrant:image_dense=0.5 \
  --max-chunk-strategy-excluded-target-hit-rate visual_asset_text=0.0 \
  --max-retrieval-role-excluded-target-hit-rate child=0.0 \
