...
chunking-docs qdrant-hybrid-search "policy corridor" \
  --package-dir outputs/package \
  --location ':memory:' \
  --vector-names text_dense,caption_dense,triple_dense \
  --filter kind=text \
...
Qdrant vector ablation modes include `text`, `caption`, `image`, `triple`, `text_caption`, `text_triple`, `text_image`, `caption_image`, `all`, `all_with_triple`, `text_caption_graph`, `text_triple_graph`, `all_graph`, and `all_with_triple_graph`. Image modes require an `image_dense` record file and a compatible image-query encoder. Triple modes require `triple_dense` records and use the text query encoder.
